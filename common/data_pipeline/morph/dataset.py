...
class DatasetLoader(DatasetLoaderBase):
    ...
    def get_directory(self) -> str:
        return f"/home/ubuntu/cluster/nbl-users/Shreyas-Sushrut-Raghu/paper_datasets/{self.printer}"

    def get_name(self) -> str:
        return self.printer + "_" + self.morph_type

    def _list_dataset(self, split_type: str) -> List[DatasetObject]:
        result: List[DatasetObject] = []
        for class_id, class_name in enumerate(self.classes):
            directory: str = os.path.join(self.get_directory(), class_name)
            if class_name == "morph":
                directory = os.path.join(directory, self.morph_type)
            directory = os.path.join(directory, split_type)
