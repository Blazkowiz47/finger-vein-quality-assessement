...
class DatasetLoaderBase:
    ...
    def __init__(
        ...
        self.total_files: Dict[DatasetSplitType, int] = {} 
        self.compiled_sets: Dict[DatasetSplitType, Any] = {}
    ...
    def _convert_to_numpy(self, split_type: DatasetSplitType, data: DatasetObject)->Any:
    ...
    def compile_sets(self) -> Dict[DatasetSplitType, Any]:
        ...
        for set_type, files in self.files.items():
            self.total_files[set_type] = len(files) * (self.augment_times + 1)

        result: Dict[DatasetSplitType, Any] = {}
        ...
        self.compiled_sets = result
        return result

    def _sample(self, data: List[Any], portion: float):
        return random.sample(data, k=int(portion * len(data)))

    def _populate_datasets_from_individual_pipelines(self) -> None:
        """
        Populates from individual train test and validation pipelines.
        """
        self.files[DatasetSplitType.TRAIN] = self.get_train_files()
        self.files[DatasetSplitType.TEST] = self.get_test_files()
        self.files[DatasetSplitType.VALIDATION] = self.get_validation_files()

    def _populate_datasets_from_all_files(self) -> None:
        """Pupulates train and test files from all_files"""
        all_files = self.get_files()
        included_files = self._sample(all_files, self.included_portion)
